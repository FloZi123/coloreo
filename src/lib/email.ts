...
const FROM = process.env.EMAIL_FROM ?? "Coloreo <noreply@example.com>";
...
export async function sendOrderConfirmation(opts: {
  ...
  if (!client) {
    console.warn("[email] RESEND nicht konfiguriert – Bestätigung übersprungen für", opts.email);
    return;
  }
  await client.emails.send({
    from: FROM,
    to: opts.email,
    subject: de ? `Deine Coloreo-Bestellung ${opts.orderNumber}` : `Your Coloreo order ${opts.orderNumber}`,
    html,
  });
}
...
export async function sendFreebieEmail(opts: { email: string; locale: Locale; downloadUrl?: string }): Promise<void> {
  ...
  const html = layout(de ? "Deine Gratis-Malvorlagen 🎁" : "Your free coloring pages 🎁", `${cta}<p style="color:#8b8499;font-size:13px">${de ? "Schau dich gern im Shop um – mit Bundles sparst du bis zu 40 %." : "Browse the shop – save up to 40% with bundles."}</p>`);
  if (!client) {
    console.warn("[email] RESEND nicht konfiguriert – Freebie übersprungen für", opts.email);
    return;
  }
  await client.emails.send({
    from: FROM,
    to: opts.email,
    subject: de ? "Deine Gratis-Malvorlagen von Coloreo" : "Your free coloring pages from Coloreo",
    html,
  });
}
