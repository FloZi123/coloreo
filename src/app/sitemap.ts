...
import { getCategories, getBooks, getBundles } from "@/lib/data";

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const base = process.env.NEXT_PUBLIC_SITE_URL ?? "http://localhost:3000";
  const [categories, books, bundles] = await Promise.all([getCategories(), getBooks({ limit: 500 }), getBundles()]);

  const entries: MetadataRoute.Sitemap = [];
  const staticPaths = ["", "/kategorien", "/bundles", "/bundle-builder", "/gratis", "/bibliothek"];
